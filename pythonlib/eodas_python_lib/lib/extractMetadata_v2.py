...
def hdr2xml(hdrfile):
    '''
    this function handel the special case of hdr file which is a repetitive xml
    hdr2xml(hdrfile)->(xmlString,error_code)
    :param hdrfile:
   
    '''
    if not isfile(hdrfile):
        return None, 101
    xml=[]
...
class extractMetadata(object): 
    ...
    def __init__(self, file_path=None, input_format='xml'):
        ...
        elif input_format == 'hdr':
            xmlstring=hdr2xml(file_path)
            self.dic, self.err = self.xml(xmlstring,fromstring='yes')
        else:
