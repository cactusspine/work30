...
def main():
	...
	if not file_metadata_list:
		err_msg = "warning: product {} no metatadata file matching the pattern was found".format(PRODUCT_ID)
		print err_msg
		report_error("direct","warning","finding_metadata",str(PROCESSING_DIR+'/testzipdir2/'+pattern),err_msg)
		query = 'INSERT INTO eodas.product_x_flag (product_id, flag_type_id, flag_reason_id) VALUES ({},1,45)'.format(PRODUCT_ID)
		cursor.execute(query)
		conn.commit()
		
		platformId=4511
		
		PRD_PLATFORM_NAME = PRD_PLATFORM_CODE =PRD_PLATFORM_SHORTNAME=PRD_PLATFORM_ID='NULL'
		
		PRD_SENSOR_NAME = PRD_SENSOR_MODE = PRD_SENSOR_TYPE =PRD_SENSOR_RESOLUTION = PRD_RESOLUTION_UNIT = 'NULL'
		
		PRD_CENTER_CODE	= PRD_CENTER_NAME = PRD_CENTER_CODE_IN_PRD_NAME = 'NULL'

		PRD_PROCESSING = PRD_GENERATION_DATE = 'NULL'

		PRD_ACQUISITION_START_TIME 	= PRD_ACQUISITION_STOP_TIME = 'NULL'

		PRD_MISSION_PHASE = PRD_IDENTIFIER = PRD_SWATH = PRD_FILE_CLASS ='NULL'

		PRD_VERSION = PRD_SUBDIR = 'NULL'

		PRD_ORBIT = PRD_ORBIT_DIRECTION = 'NULL'
		
		PRD_FOOTPRINT = PRD_INFO = PRD_QUALITY = 'NULL' 
		
		PRD_TRACK_MIN = PRD_TRACK_MAX = PRD_FRAME_MIN = PRD_FRAME_MAX = 'NULL'

	
		PRD_YEAR = PRD_MONTH = PRD_DAY = 'NULL'
		...
		try:
			...
			err_msg = "warning: product {} metatadata file is empty".format(PRODUCT_ID)
			print err_msg
			report_error("direct","warning","finding_metadata",str(PROCESSING_DIR+'/testzipdir2/'+pattern),err_msg)
			query = 'INSERT INTO eodas.product_x_flag (product_id, flag_type_id, flag_reason_id) VALUES ({},1,46)'.format(PRODUCT_ID)
			cursor.execute(query)
			conn.commit()
	...
	# ----------------------------------------------------------------------------------
	if PRD_DAY != 'NULL' and PRD_MONTH !='NULL' and PRD_YEAR!='NULL':
